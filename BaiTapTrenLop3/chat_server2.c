...
int main() {
    ...
    while (1) {
        ...
        if (client_count < 2) {
            // Lưu thông tin client và tạo một tiến trình riêng để xử lý
            clients[client_count].client_socket = client_socket;
            pthread_create(&clients[client_count].thread_id, NULL, handle_client, &clients[client_count]);
            client_count++;
            ...
            // Đã có đủ 2 client, từ chối kết nối mới và đóng kết nối
